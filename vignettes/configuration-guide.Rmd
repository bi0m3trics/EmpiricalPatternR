...
Every simulation in **EmpiricalPatternR** is driven by a *configuration list* that bundles four components:
...
This vignette explains every entry in detail so you can create, modify, and validate configurations for any forest type.
...
The optimizer converts this to an expected tree count for the plot: $n = \text{density\_ha} \times \text{plot\_size}^2 / 10\,000$.
...
Names **must** match the species codes used in allometric parameter lists (e.g., `"PIED"`, `"JUSO"`, `"PIPO"`).
Must sum to 1.0.
...
Initial trees are drawn from $\text{Normal}(\text{mean\_dbh},\, \text{sd\_dbh})$, truncated at a 5 cm minimum.
...
-   Set **density**, **species**, and **canopy_cover** high (60--80) — these are directly observed field metrics.
-   Keep **ce**, **dbh_mean**, and **height** low (1--20) — these tend to emerge naturally from allometry and density.
...
Multiplicative cooling factor applied each iteration: $T_{i+1} = T_i \times \text{cooling\_rate}$.
...
See `?get_default_allometric_params` and `?get_ponderosa_allometric_params` for the shipped parameter sets.
...
-   Required target fields present (`density_ha`, `species_props`, `species_names`, `canopy_cover`, `cfl`)
