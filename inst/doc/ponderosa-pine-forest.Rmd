---
title: "Ponderosa Pine Forest Simulation"
author: "Andrew Sánchez Meador"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Ponderosa Pine Forest Simulation}
...
This vignette demonstrates how to create **custom configurations** for forest
types other than pinyon-juniper.
We simulate a ponderosa pine dominated mixed-conifer forest with:

- Custom allometric parameters for larger trees
- Multiple species (PIPO, PSME, ABCO)
- Regular spacing (managed forest)
- No nurse tree effect

## Why Custom Configurations?

The pre-built `pj_huffman_2009()` configuration is designed for pinyon-juniper
woodlands.
Other forest types differ substantially:

| Characteristic | Pinyon-Juniper | Ponderosa Pine |
|----------------|----------------|----------------|
| Density | ~900 trees/ha | ~450 trees/ha |
| Mean DBH | 20 cm | 35 cm |
| Mean Height | 6.5 m | 18 m |
| Species | 2 (PIED, JUSO) | 3+ (PIPO, PSME, ABCO) |
| Spacing | Random (R ~1.0) | Regular (R ~1.4) |
| Nurse Effect | Yes | No |
...
## Step 2: Examine Ponderosa Allometric Parameters

Species-specific allometric equations control crown, height, and foliage
relationships.
The package ships with a ponderosa pine parameter set:
...
Compare tree dimensions for a 40 cm DBH ponderosa pine under different
parameter sets:
...
## Step 3: Build a Custom Configuration

Use `create_config()` to assemble all parameters:
...
When `save_plots = TRUE`, `analyze_simulation_results()` writes:

| File | Contents |
|------|----------|
| `*_all_trees.csv` | All trees (live + dead) with full attributes |
| `*_live_trees.csv` | Live trees only |
| `*_history.csv` | Optimization convergence history |
| `*_summary.csv` | Summary statistics (simulated vs. target) |
| `*_plots.pdf` | Spatial pattern and diameter distribution plots |

## References

Grier, C.C., Elliott, K.J., & McCullough, D.G. (1992).
Biomass distribution and productivity of *Pinus edulis*--*Juniperus
monosperma* woodlands of north-central Arizona.
*Forest Ecology and Management*, 50, 331--350.

## See Also

- `vignette("pinyon-juniper-woodland")` --- pre-built P-J workflow
- `vignette("configuration-guide")` --- full config reference
- `?create_config` --- configuration builder
